...
- **profiles** (multi-perfil): `id, name UNIQUE, exchange, lighter_account_index, lighter_api_key_private, lighter_api_key_index, hyperliquid_address, hyperliquid_secret, created_at, updated_at` — cada perfil tem suas próprias credenciais; trades/signals/logs/config são isolados por `profile_id`
...
- **Migration M8 (`_migrate_to_multi_profile`)** — one-shot na `init_db`, roda DEPOIS de M6/M7. **M8a:** adiciona coluna `profile_id INTEGER DEFAULT 1` em `trades`, `signals`, `logs` + índices. **M8b:** (1) cria perfil `id=1, name="Default"` populado a partir das credenciais globais legadas (`account_address`, `secret_key`, `lighter_account_index`, `lighter_api_key_private`, `lighter_api_key_index`); (2) namespaceia keys per-perfil em `profile.1.<key>` — prefixos `strategy.`, `risk.`, `sizing.` + exact keys `bot_status`, `assets`, `lighter.client_order_counter`; (3) deleta as keys originais + credenciais legadas do `config` (agora vivem na row do perfil); (4) backfill `profile_id=1` em rows com `profile_id IS NULL`. Mantém intocadas: `selected_exchange`, `use_lighter_ws_candles`, `last_ts.*`, markers `_migration_*`, `flask.secret_key`. Idempotente via marker `_migration_multi_profile=done`. Helper `_is_m8_profile_key(key)` decide o que migrar; constants `_M8_GLOBAL_KEYS`, `_M8_PROFILE_PREFIXES`, `_M8_PROFILE_EXACT_KEYS` no topo do arquivo
