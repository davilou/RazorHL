...
- **Config per perfil (risk/sizing/slippage/assets):** `db._PER_PROFILE_CFG_KEYS` lista as 11 keys que vivem por perfil — `monitored_assets`, `max_positions`, `max_daily_loss_pct`, `sizing_mode`, `size_mode`, `risk_pct_per_trade`, `fixed_trade_size_usd`, `slippage`, `tp_atr_multiplier`, `sl_atr_multiplier`, `use_testnet`. Demais (debug_logging, selected_exchange, fee_rate_round_trip, flask.secret_key, credenciais legadas) ficam globais. **Helper `db.get_effective_config(profile_id)`** lê os globais e sobrepõe com `profile.<id>.<key>` para as keys per-perfil. **`/api/config` GET** retorna effective do perfil ativo; **POST** separa o payload: per-perfil → `set_profile_configs`, globais → `set_configs`. **RiskManager._cfg()** e `_build_cfg_for_profile` em main.py usam `get_effective_config(self.profile_id)`. Sem isso, mudar slippage/max_positions/etc. na UI gravava no global e afetava todos os perfis. Backfill manual copiou os 8 valores globais existentes pra `profile.1.*` e `profile.3.*` como ponto de partida (após isso, mudanças em um perfil ficam isoladas). `_union_assets` agora lê `profile.<pid>.monitored_assets` primeiro (key que o config UI grava), com fallback para `profile.<pid>.assets` (legacy M8) e finalmente o global `monitored_assets`

- **`profile_id` propagado pra estratégia (blocked signals):** `evaluate_all` passa `profile_id=profile_id` como kwarg pra `strategy.evaluate(...)`. As estratégias colocam `"profile_id": int(kwargs.get("profile_id", 1))` no dict `base` que serve de template pros `db.insert_signal(base, ...)` diretos (caminho de blocked LONG/SHORT em `bb_reversion.py:172/177/199/204`). Sem isso, todos os sinais bloqueados por EMA/RSI filter caíam em `profile_id=1` (default do `insert_signal.setdefault`), mesmo quando a estratégia estava enabled SOMENTE no perfil "15 min". Sinais normais (retornados pelo evaluate, sem insert direto) já tinham `profile_id` setado explicitamente pelo loop em `process_asset`
