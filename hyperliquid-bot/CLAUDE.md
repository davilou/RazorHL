...
9. **Scanner v2** (`/scanner_v2`): Aba dedicada para o `bot/backtest/scanner_v2.py`. Dois sub-modos via tabs: **Grid Scan** (mesma UX do `/scanner` mas com 3 colunas novas — ADX (`14/<20` se ativo, `off` caso contrário), Sess (`13-21h` ou `24h`), ATR (`tp_mult/sl_mult` ou `%`) — e filtros adicionais no painel lateral para isolar combos com ADX/sess/ATR on vs off) e **Walk-Forward** (form com total_days/n_windows/train_ratio/top_n/max_combos, KPIs no topo incluindo `wfo_efficiency` colorizado por threshold ≥0.7=verde / ≥0.5=neutro / <0.3=vermelho com label textual "Excelente/Bom/Marginal/Overfit", tabela por janela com barra de eficiência por linha, e `<details>` colapsável por janela mostrando top params IS vs ROI/trades OOS). Botão "Aplicar" em ambos os modos reutiliza `apply_result` do scanner antigo via `POST /api/scanner_v2/apply` — os 7 campos novos (`adx_period`, `adx_min`, `session_start`, `session_end`, `atr_tp_mode`, `atr_tp_mult`, `atr_sl_mult`) ficam preservados em `scanner_metrics.scanner_params` mas NÃO viram params live (a estratégia em produção ainda usa só os params base + TP/SL %); o modal de apply alerta explicitamente sobre isso. Endpoints: `GET /api/scanner_v2/assets?timeframe=`, `POST /api/scanner_v2/run` (grid scan async via `start_scan_v2_job`), `POST /api/scanner_v2/wfo` (WFO async via `start_wfo_job`), `GET /api/scanner_v2/status/<job_id>` (mesmo formato dos jobs do scanner antigo: status running|done|error + progress + result/error), `POST /api/scanner_v2/apply`. Job registry compartilhado entre grid e wfo em `scanner_v2._jobs` (key `kind: "scan"|"wfo"`). `scanner_v2_page` excluída do `check_configured` redirect (mesmo padrão do scanner antigo).
